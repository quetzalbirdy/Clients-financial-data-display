  ...
  const obtenerRecepciones =  async () => {    
    await servicios.getRecepcionesList(tipo/* , true */).then((res) => {     
      ...
      for (var i = 0; i < resOrdenado.length; i++) {
        ...
        if (/* (recepcion.tipo === 'MIGO_GR' && recepcion.estado !== 'Facturada') || (recepcion.tipo === 'ML81N' && recepcion.estado === 'Aceptada') */recepcion.estado === 'Aceptada') {
  ...
  return(
    <div className="fondo-app sin-facturar compra con-bottom-bar flex flex-column">
      ...
      <div className={`contenido-ventana${filtroActive ? ' filtro-active' : ''}`}>
        ...
        <ValorTotal texto={'Total sin facturar'} valorTotal={totalSinFacturar !== '' ? totalSinFacturar.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1.") : ''}/>   
